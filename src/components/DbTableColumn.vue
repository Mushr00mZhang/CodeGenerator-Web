<template>
  <ElForm ref="columnFormRef" :model="newColumn" :rules="rules" label-width="auto">
      ...
      <ElCol :span="12">
        <ElFormItem label="数据类型" prop="type">
          <ElSelect
            ...
            @change="typeChanged"
          >
            <template v-for="t in newColumn.isPrimary ? dbTableColumnPkTypes : dbTableColumnTypes">
    ...
    <ElRow>
      <ElCol :span="12">
        <ElFormItem label="获取字段" prop="isGetProp">
    ...
    <!-- <ElFormItem label="导入字段" prop="isImportProp">
      <ElRadioGroup v-model="newColumn.isImportProp">
        <ElRadioButton :value="true">是</ElRadioButton>
        <ElRadioButton :value="false">否</ElRadioButton>
      </ElRadioGroup>
    </ElFormItem> -->
